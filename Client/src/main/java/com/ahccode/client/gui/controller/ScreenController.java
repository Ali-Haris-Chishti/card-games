...
import com.ahccode.client.game.thulla.ThullaController;
import com.ahccode.client.gui.component.CardInHandBox;
import com.ahccode.client.gui.component.CardInStackBox;
    ...
    public void cardClicked(Card card, int playerNum) {
        if (gameController.cardSelected(card, playerNum)) {
            try {
                ...
                ClientContext.currentPlayer.getAssociatedClient().send(new Message(MessageType.MOVE_CARD, new CardMessage(card.getCardFamily(), card.getCardNumber(), ClientContext.currentPlayer.getPlayerNumber())));
            } catch (IOException e) {
